    ...
    while True:
        try:
            ...
            for item in image_urls:
                used_links.append(item)  # Keep track of used links
                try:
                    response = requests.get(item, stream=True, timeout=10)
                    if response.status_code == 200 and 'image' in response.headers.get('Content-Type', ''):
                        id = generate_unique_id()
                        file_path = f"{path}{id}.png"
                        with open(file_path, 'wb') as file:
            ...
            num *= 2
            if num > 10:
                if len(query.split()) > 1:  # Reduce the query complexity if possible
                    words = query.split()
                    query = ' '.join(words[:-1])
                    num = 2  # Reset num
                else:
...
def download_video(query, path, initial_length=600, max_attempts=10):
    """
...
def createSceneSimple(instruction, start_time, end_time):
    print(instruction)
    global a
    global b
    global c
    ...
    if (clip.size[1] / clip.size[0]) > 1.5 and random.randint(1,2+a)<2:
        print("STYLE 1")
        clip = aspectRatio(clip, (random.randint(3,7)/10)*1080, 1920)
        clip = clip.set_position(('center', 'center'))
        clips.append(clip)
        a+=2
     
    elif (clip.size[0] / clip.size[1]) > 1.5 and random.randint(1,2+b)<2:
        print("STYLE 2")
    ...
    #SIDE IMAGES
    for secondary in instruction[1]:
      inst = secondary+" PNG"
      link = search_and_download_image(inst, "/Users/tassiloar/Desktop/Nexamedia/HEAP/visual/")
      sizex = random.randint(500,600)
      clip = createimageclip(link, random.randint(-300,300), random.randint(-300,300),sizex,sizex, start_time, end_time)
      clips.append(clip)
...
def createimageclip(link, x_pos, y_pos, wdith, height, start_time, end_time):
  ...
  clip = clip.rotate(random.randint(-20,20))
  
  clip = clip.set_position(((1080/2) - (clip.size[0]/2) + x_pos , (1920/2) - (clip.size[1]/2) + y_pos + 200 ))
...
instructions = [['I Kylie Jenner billionaire', [' billionaire']], ['V Kylie Jenner self-made', [' Kylie Jenner']], ['V Kylie Jenner Kardashians realityTv', [' Kardashians']], ['V Kylie Jenner twist wealth', ['Kylie Jenner']], ['I Kylie Jenner Cosmetics beauty brand', [' Kylie Cosmetics']], ['V Kylie Jenner lip kit brand', [' lip kit']], ['V Kylie Jenner social media', [' social media']], ['V Kylie Jenner monthsKylieCosmetics sales', ['Kylie Jenner']], ['V Kylie Jenner social media billionaire', [' social media']], ['I Kylie Jenner lip kits', []], ['V Kylie Jenner Cosmetics eyeliners', ['Kylie Jenner']], ['V Kylie Jenner Coty Inc jackpot', [' Coty Inc']], ['V Kylie Jenner controversies queen', []], ['I Kylie Jenner Cosmetics backyard', [' Kylie Cosmetics']]]
...
background = addbackgrund(0, times[len(times)-1][1])

#background = background.subclip(0,5)
...
#while s < 6:  
  clip_temp = createSceneSimple(instructions[s], times[s][0], times[s][1])
