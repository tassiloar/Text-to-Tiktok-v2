...
def createSceneSimple(instruction, start_time, end_time, wordsbase, icons, icons_color, full_screen_img, full_screen_vid, half_screen_img, half_screen_vid, content_color, swoosh_sound_effect, pop_sound_effect, fade_in_transition, cold_cut_transition, swoosh_transition, main_figures, section): 
    ...
    if icons == True:
      #SIDE IMAGES
      ff = 0
      while ff < len(instruction[1]) and ff < 3:
        ...
        if not link == None:
          ...
          clip = createimageclip(link, rotate, x_pos, y_pos,sizex,sizex, float(start_time)+starttt, float(end_time), swoosh_sound_effect, pop_sound_effect, fade_in_transition, cold_cut_transition, swoosh_transition,is_png)  
...
def find_section_bound(words, setions):
    ...
    while x < len(section_times)-1:
      ...
      if int(section_times[x][1]) < int(section_times[x][0]):
        avg = (float(section_times[x][0]) + float(section_times[x+1][1]))/2
        section_times[x][1] = avg
        section_times[x+1][0] = avg
...
def instruction(sectionss, topic):
  ...
  for section in sections:
    ...
    def contains_capital_letter(s):
      for char in s:
          if char.isupper():
              return True
      return False

    if not contains_capital_letter(primary):
  ...
  "parameters": {
    ...
    "properties": {
      ...
      "included_images": {
        ...
        "description": "This is a list of image prompts (not links) that the user wants included in the video. For example: 'drake at the grammys'"
      },
       "included_videos": {
        "type": "array",
        "items": {
        ...
        "description": "This is a list of video prompts (not links) that the user wants included in the video. For example: 'skiing backflip'"
      },
      "length_min": {
        "type": "integer",
      ...
      "voice": {
        ...
        "description": "Name of the voice narrator to use. Do not initialize unless directly adressed by user. Use Drake voice"
      }
    },
    "required": [
      "topic",
...
input = "Create a video that is 1 minute long on how kanye west made yeezy the company. Use popups. Use footage from this video https://www.youtube.com/watch?v=a39UW155IQY and this images of Kanye West and his daughter"
...
Params = {'topic': 'How Kanye West made Yeezy the company', 'included_links': ['https://www.youtube.com/watch?v=a39UW155IQY'], 'included_images': ['Kanye West', 'Kanye West and his daughter'], 'length_min': 1, 'background_color': [255, 100, 100], 'main_figures': ['Kanye West', 'Yeezy'], 'icons': True, 'voice': "drake"}
print(Params)
...
text = OpenAIAssistant("asst_7zYo27j8yxn6xLBdcM5rzAJr", f"Follow your instructions to create a new text on {topic} between {length_min*210} and {length_min*230} words long. Spesifications: {specs}")
#text = 
print("text")
print(text)
print()
...
audio = unreal_speech(voice, text)
...
background = addbackgrund(backgroun_graphic, background_color,0, times[len(times)-1][1])
